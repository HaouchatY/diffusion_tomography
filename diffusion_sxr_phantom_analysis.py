...
for sigma_y_tomo in noise_levels:
    for procedure in procedures:
        ...
        csv_name = f"metrics_diffusion_sxr_{sigma_y_tomo}_{proc_str}.csv"
        csv_path = csv_dir / csv_name
        df.to_csv(csv_path, index=False)
        print(f"   saved {csv_path}")
        print(df.describe()[["psnr", "ssim", "mse"]])
        np.save(csv_dir / f"post_means_sxr_{sigma_y_tomo}_{proc_str}.npy", post_means)
        np.save(csv_dir / f"post_stds_sxr_{sigma_y_tomo}_{proc_str}.npy", post_stds)