...
def run_study(phantom_indices):
    ...
    test_samples = np.load(samples_dir / "sxr_samples_with_background_coarse.npy")[samples_idxs]  # (1000, H, W)
    psis         = np.load(samples_dir / "psis_coarse.npy")[samples_idxs]
    trim_vals    = np.load(samples_dir / "trimming_values.npy")[samples_idxs]
    ...
    # --------------------------------------------------------------------------
    noise_realizations = np.load("noise_realizations_sxr.npy").astype(np.float32)  # (1000, n_channels)
    scaling_params     = np.load("scaling_params.npy").astype(np.float32)           # (1000,)
    ...
    csv_dir = Path("metrics_csv_phantom_analysis_sxr_rd")
    ...
    for sigma_y_tomo in noise_levels:
        for procedure in procedures:
            ...
            csv_name = f"metrics_rd_sxr_{sigma_y_tomo}_{proc_str}.csv"
            ...
            for i in range(num_eval_samples):
                ...
                # ------------------------------------------------------------------
                # Redefine functionals with tuned alpha; fresh mask copy
                # ------------------------------------------------------------------
                mask_core = tomo_helps.define_core_mask(
                    psi=psi, dim_shape=reconstruction_shape, trim_values_x=trim_val_
                )
                f, g = fct_def.define_loglikelihoodfromdata_and_logprior(
                    ...
                    sigma_err=sigma_used,
                    reg_fct_type=reg_fct_type,
                    alpha=best_anis_param,
                    sampling=sampling_param,
                    seed=i,
                )
                g = hyper_tune._redefine_anis_param_logprior(
                    g, anis_param_data["CV_full_random"][0, best_anis_idx]
                )
                ...
                np.save(
                    csv_dir / f"results_sxr_{sigma_y_tomo}_{proc_str}_{i}.npy",
                    sample_results,
                    allow_pickle=True,
                )
