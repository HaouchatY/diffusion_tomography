...
if diagnostic=="sxr":
    ...
    lambda_rd_values = np.logspace(-9, -2, 8)
...
for sigma_y_tomo in noise_levels:
    ...
    if rd_by_camera:
        csv_name = f"metrics_csv_phantom_analysis_rd/metrics_rd_pilatus_rd_alpha_by_camera_{sigma_y_tomo}_false.csv"
    else:
    ...
    for clip in clip_values:
        for zeta in zeta_values:
            for lambda_ in lambda_values:
                for lambda_rd_ in lambda_rd_values:

                    clip_suffix = f"_clip{clip}" if diagnostic == "pilatus" else ""
                    if rd_by_camera:
                        csv_name = f"metrics_diffusion_{diagnostic}_{sigma_y_tomo}_zeta{zeta}_lambda{lambda_}_lambda_rd{lambda_rd_}_by_camera{clip_suffix}.csv"
                    else:
                        csv_name = f"metrics_diffusion_{diagnostic}_{sigma_y_tomo}_zeta{zeta}_lambda{lambda_}_lambda_rd{lambda_rd_}{clip_suffix}.csv"
                    csv_path = csv_dir / csv_name
                    if csv_path.exists():
                        print(f"Already exists {csv_path}, skipping...")
                        continue

                    print(f"-- sigma={sigma_y_tomo}, zeta={zeta}, lambda_={lambda_}, lambda_rd={lambda_rd_}, clip={clip}"+f", by camera"*(rd_by_camera))
